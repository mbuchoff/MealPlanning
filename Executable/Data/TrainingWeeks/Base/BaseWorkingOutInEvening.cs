using static SystemOfEquations.FoodGrouping;

namespace SystemOfEquations.Data.TrainingWeeks.Base;

internal record BaseWorkingOutInEvening : TrainingWeekBase
{
    internal BaseWorkingOutInEvening(decimal targetGramsProteinPerDay) : base(
        "Base",
        nonworkoutMeals:
        [
            new Meal("Waking",
            new(P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 10, C: 25),
                new FoodGrouping("Blueberry oatmeal shake",
                    [Foods.BlueBerries_1_Scoop * 3, Foods.AlmondMilk_2_Cup],
                    Foods.PeaProtein_1_Scoop,
                    Foods.ChiaSeeds_2_5_Tbsp,
                    Foods.Oats_1_Scoop,
                    PreparationMethodEnum.PrepareAsNeeded)),
            new Meal("3-5 hours after last meal",
            new(P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 20, C: 25),
            FoodGroupings.Ezekiel),
        new("3-5 hours after last meal",
            new(P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 20, C: 25),
            FoodGroupings.TofuAndWheatBerries),
        new ("3-5 hours after last meal",
            new(P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 20, C: 25),
            FoodGroupings.TofuAndWheatBerries),
        new Meal("Bedtime",
            new Macros(P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 25, C: 0),
            new FoodGrouping("Shake",
                [Foods.AlmondMilk_2_Cup],
                Foods.PeaProtein_1_Scoop,
                Foods.AlmondButter_1_Tbsp,
                Foods.FatToCarbConversion,
                PreparationMethodEnum.PrepareAsNeeded)),
        ],
        runningMeals:
        [
            new Meal("Waking",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 20),
             new FoodGrouping("blueberries, oatmeal, and edamame",
                [Foods.BlueBerries_1_Scoop],
                Foods.Edamame_1_Scoop,
                Foods.AlmondButter_1_Tbsp,
                Foods.Oats_1_Scoop,
                PreparationMethodEnum.PrepareAsNeeded)),
        new Meal("Meal 2",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 20),
            FoodGroupings.Ezekiel),
        new("Meal 3",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 20),
            FoodGroupings.Seitan),
        new("Meal 4",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 40),
            FoodGroupings.Seitan),
        new("1/2 shake during working, 1/2 right after",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 65),
            FoodGroupings.WorkoutShake),
        new Meal("Bedtime",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 55),
            FoodGroupings.EnglishMuffinsAndPasta(englishMuffins: 2)),
        ],
        xfitMeals:
        [
            new Meal("Waking",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 25),
             new FoodGrouping("blueberries, oatmeal, and edamame",
                [Foods.BlueBerries_1_Scoop * 2],
                Foods.Edamame_1_Scoop,
                Foods.AlmondButter_1_Tbsp,
                Foods.Oats_1_Scoop,
                PreparationMethodEnum.PrepareAsNeeded)),
        new Meal("Meal 2",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 25),
            FoodGroupings.Ezekiel),
        new("Meal 3",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 25),
            FoodGroupings.Tofu),
        new("Meal 4",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 50),
            FoodGroupings.Tofu),
        new("1/2 shake during working, 1/2 right after",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 65),
            FoodGroupings.WorkoutShake),
        new Meal("Bedtime",
            new(P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 20, C: 80),
            FoodGroupings.EnglishMuffinsAndPasta(englishMuffins: 3)),
        ])
    {

    }
}

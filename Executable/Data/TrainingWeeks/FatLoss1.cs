using static SystemOfEquations.FoodGrouping;

namespace SystemOfEquations.Data.TrainingWeeks;

internal record FatLoss1 : TrainingWeekBase
{
    internal FatLoss1(decimal targetGramsProteinPerDay) : base(
        "Base",
        nonworkoutMeals:
        [
            new Meal("Waking",
                new(P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 0, C: 25),
                BlueBerryShakeTwice),
            new Meal("3-5 hours after last meal",
                new (P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 0, C: 25),
                BlueBerryShakeTwice),
            new Meal("3-5 hours after last meal",
                new (P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 0, C: 25),
                JustTofu),
            new Meal("3-5 hours after last meal",
                new (P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 10, C: 25),
                JustTofu),
            new Meal("Bedtime",
                new Macros(P: FatLossProteinPerMealOnNonworkoutDay(targetGramsProteinPerDay), F: 25, C: 0),
                new FoodGrouping("Shake",
                    [Foods.AlmondMilk_2_Cup],
                    Foods.PeaProtein_1_Scoop,
                    Foods.AlmondButter_1_Tbsp,
                    Foods.FatToCarbConversion,
                    PreparationMethodEnum.PrepareAsNeeded)),
        ],
        runningMeals:
        [
            new Meal("Waking",
            new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 20),
                BlueBerryShakeTwice),
            new Meal("Meal 2",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 20),
                    BlueBerryShakeTwice),
            new Meal("Meal 3",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 20),
                new FoodGrouping("Edamame",
                    Foods.Edamame_1_Scoop,
                    Foods.FatToCarbConversion,
                    Foods.ProteinToCarbConversion,
                    PreparationMethodEnum.PrepareAsNeeded)),
            new Meal("Meal 4",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 15, C: 40),
                Seitan),
            new Meal("1/2 shake during working, 1/2 right after",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 65),
                FoodGroupings.WorkoutShake),
            new Meal("Bedtime",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 15, C: 55),
                FoodGroupings.Seitan),
        ],
        xfitMeals:
        [
            new Meal("Waking",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 25),
                BlueBerryShakeTwice),
            new Meal("Meal 2",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 25),
                BlueBerryShakeTwice),
            new Meal("Meal 3",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 25),
                JustEdamame),
            new Meal("Meal 4",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 15, C: 50),
                Tofu),
            new Meal("1/2 shake during working, 1/2 right after",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 0, C: 65),
                FoodGroupings.WorkoutShake),
            new Meal("Bedtime",
                new (P: FatLossProteinPerMealOnWorkoutDay(targetGramsProteinPerDay), F: 15, C: 80),
                Tofu),
        ])
    {
    }

    private static readonly FoodGrouping JustTofu = new(
        "Tofu",
        Foods.Tofu_91_Grams,
        Foods.FatToCarbConversion,
        Foods.ProteinToCarbConversion,
        PreparationMethodEnum.PrepareInAdvance);

    private static readonly FoodGrouping BlueBerryShakeTwice = new(
        "Blueberry shake (make twice as much)",
        Foods.PeaProtein_1_Scoop,
        Foods.FatToCarbConversion,
        Foods.BlueBerries_1_Scoop,
        PreparationMethodEnum.PrepareAsNeeded);

    private static readonly FoodGrouping Seitan = new(
        "seitan",
        Foods.Seitan_Sprouts_Yeast_1_Gram_Gluten_4x,
        Foods.FatToCarbConversion,
        Foods.BrownRice_45_Grams,
        PreparationMethodEnum.PrepareInAdvance);

    private static readonly FoodGrouping JustEdamame = new(
        "Edamame",
        Foods.Edamame_1_Scoop,
        Foods.FatToCarbConversion,
        Foods.ProteinToCarbConversion,
        PreparationMethodEnum.PrepareAsNeeded);

    public static FoodGrouping Tofu { get; } = new(
        "tofu",
        Foods.Tofu_91_Grams,
        Foods.FatToProteinConversion,
        Foods.BrownRice_45_Grams,
        PreparationMethodEnum.PrepareInAdvance);
}
